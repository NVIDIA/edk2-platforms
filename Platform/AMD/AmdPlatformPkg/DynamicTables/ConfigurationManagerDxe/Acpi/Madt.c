...
UINT8  *mMadtCcdOrder = NULL;
UINT8  mMadtCcdCount  = 0;
  ...
  )
{
  ...
  Left  = (CONST EFI_PROCESSOR_INFORMATION *)ProcessorInfoLeft;
  Right = (CONST EFI_PROCESSOR_INFORMATION *)ProcessorInfoRight;

  // Get the CCD Index number
  LeftCcdIndex = MAX_UINT32;
  for (Index = 0; Index < mMadtCcdCount; Index++) {
  ...
  )
{
  ...
  if ((MAX_UINTN / NumberOfProcessors) > sizeof (EFI_PROCESSOR_INFORMATION)) {
  ...
  for (Index = 0; Index < NumberOfProcessors; Index++) {
    ...
    if (ProcessorInfoBuffer[Index].ExtendedInformation.Location2.Die > NumCcd) {
      NumCcd = ProcessorInfoBuffer[Index].ExtendedInformation.Location2.Die;
    }
  }
  ...
  mMadtCcdCount = PcdGetSize (PcdAmdNumaCcdOrder) / sizeof (UINT8);
  if (mMadtCcdCount == 0) {
    DEBUG ((
      DEBUG_ERROR,
      "%a:%d PcdAmdNumaCcdOrder is empty.\n",
      __func__,
      __LINE__
      ));
    FreePool (ProcessorInfoBuffer);
  ...
  mMadtCcdOrder = AllocateCopyPool (
                    PcdGetSize (PcdAmdNumaCcdOrder),
                    (const void *)PcdGetPtr (PcdAmdNumaCcdOrder)
                    );
  if (mMadtCcdOrder == NULL) {
    DEBUG ((
      DEBUG_ERROR,
      "%a:%d Failed to allocate memory for mMadtCcdOrder. Status(%r)\n",
      __func__,
      __LINE__,
      Status
      ));
  ...
  /// Check if CCD number based reordering is enabled
  if (mMadtCcdOrder[0] != 0xFF) {
    if (mMadtCcdCount < NumCcd) {
      DEBUG ((
        DEBUG_ERROR,
        "%a:%d PcdAmdNumaCcdOrder list is smaller than system CCD count.",
        __func__,
        __LINE__
        ));
      FreePool (ProcessorInfoBuffer);
      FreePool (mMadtCcdOrder);
      ASSERT_EFI_ERROR (EFI_INVALID_PARAMETER);
      return EFI_INVALID_PARAMETER;
    }
  ...
  IntrSourceOverrideInfo[0].Flags                 = 0x0;
  ...
  /// Set MADT OemTableId from AMD PCD (see PcdAmdAcpiMadtOemTableId in AmdPlatformPkg.dec).
  for (AcpiTableIndex = 0; AcpiTableIndex < PlatformRepo->CurrentAcpiTableCount; AcpiTableIndex++) {
    if (PlatformRepo->CmAcpiTableList[AcpiTableIndex].AcpiTableSignature ==
        EFI_ACPI_6_5_MULTIPLE_APIC_DESCRIPTION_TABLE_SIGNATURE)
    {
      if (PcdGet64 (PcdAmdAcpiMadtOemTableId) != 0) {
        PlatformRepo->CmAcpiTableList[AcpiTableIndex].OemTableId =
          PcdGet64 (PcdAmdAcpiMadtOemTableId);
      } else {
        PlatformRepo->CmAcpiTableList[AcpiTableIndex].OemTableId =
