  ...
  )
{
  ...
  for (Index = 0; Index < PlatformRepo->CurrentAcpiTableCount; Index++) {
    ...
    {
      if (PcdGet64 (PcdAmdAcpiMcfgOemTableId) != 0) {
        PlatformRepo->CmAcpiTableList[Index].OemTableId = PcdGet64 (PcdAmdAcpiMcfgOemTableId);
      } else {
        PlatformRepo->CmAcpiTableList[Index].OemTableId = PcdGet64 (PcdAcpiDefaultOemTableId);
      }
