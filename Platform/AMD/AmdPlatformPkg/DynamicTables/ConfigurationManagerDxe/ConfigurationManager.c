...
#define REFERENCE_TOKEN(Field)                                    \
  (CM_OBJECT_TOKEN)((UINT8*)&mAmdPlatformRepositoryInfo +                 \
...
EDKII_PLATFORM_REPOSITORY_INFO  mAmdPlatformRepositoryInfo = {
  ...
  /// HpetInfo
  {
    FixedPcdGet32 (PcdHpetBaseAddress),
    0x37EE,
    0x0
  },
  /// Wsmt flags info
  ...
  {
    0x01,
    { EFI_ACPI_6_5_SYSTEM_IO, 0x8,  0,   EFI_ACPI_6_5_BYTE,      0x0000000000000CA2 }
  },
  /// _STA
  {
    {
  ...
  )
{
  ...
  /// set the OemId, OemTableId and OemRevision for the CmACpiTableList
  ...
  CopyMem (
    mAmdPlatformRepositoryInfo.CmInfo.OemId,
    PcdGetPtr (PcdAcpiDefaultOemId),
    sizeof (mAmdPlatformRepositoryInfo.CmInfo.OemId)
    );
  ...
  Status = UpdatePlatformCmInfo (&mAmdPlatformRepositoryInfo);
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "Platform Configuration Manager Info update failed. Status = %r\n",
      Status
      ));
  }

  /// Set the C-STATE reference token
  mAmdPlatformRepositoryInfo.CoreResources->ReferenceToken     = REFERENCE_TOKEN (CstateResources);
  mAmdPlatformRepositoryInfo.CstateResources[0].ReferenceToken = REFERENCE_TOKEN (CstInfo[0]);
  mAmdPlatformRepositoryInfo.CstateResources[1].ReferenceToken = REFERENCE_TOKEN (CstInfo[1]);
  mAmdPlatformRepositoryInfo.CsdInfo[0].CstPkgRefToken         = REFERENCE_TOKEN (CstInfo[0]);
  mAmdPlatformRepositoryInfo.CsdInfo[1].CstPkgRefToken         = REFERENCE_TOKEN (CstInfo[1]);
