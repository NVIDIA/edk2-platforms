  ...
  )
{
  ...
  Left  = (CONST EFI_PROCESSOR_INFORMATION *)LocalX2ApicLeft;
  Right = (CONST EFI_PROCESSOR_INFORMATION *)LocalX2ApicRight;

  // Get the CCD Index number
  LeftCcdIndex = MAX_UINT32;
  for (Index = 0; Index < mCcdCount; Index++) {
  ...
  )
{
  ...
  if (NumberOfCpus <= (MAX_UINTN / sizeof (EFI_PROCESSOR_INFORMATION))) {
  ...
  for (Index = 0; Index < NumberOfCpus; Index++) {
    ...
    if (mApicIdtoUidMap[Index].ExtendedInformation.Location2.Die > NumberOfCcd) {
      NumberOfCcd = mApicIdtoUidMap[Index].ExtendedInformation.Location2.Die;
    }
  }
  ...
  mCcdCount = PcdGetSize (PcdAmdNumaCcdOrder) / sizeof (UINT8);
  if (mCcdCount == 0) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  if (mCcdOrder == NULL) {
    DEBUG ((
      DEBUG_ERROR,
      "%a:%d Failed to allocate memory for mCcdOrder. Status(%r)\n",
      __func__,
      __LINE__,
      Status
      ));
  ...
  )
{
  ...
  if (PcdGet64 (PcdAmdAcpiCpuSsdtOemTableId) != 0) {
    Table->OemTableId = PcdGet64 (PcdAmdAcpiCpuSsdtOemTableId);
  } else {
