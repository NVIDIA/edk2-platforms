...
/*
 * Calculates the SPI offset of the current image using the
 * ROM OVERRIDE register bits.
 * This is calculated from the SPI Base offset which is
 * Computed using FlashSize for Turin Ax and Bx
 * 0xFC00 0000 for Turin Cx and Genoa
 * @param[in]  FlashSize -  Size of physical flash part
 * 
 * @return    SPI Offset of current ROM image.
 */
UINT32
SetSpiFlashOffset (
  UINT32 FlashSize
  )
{
  ...
  RomPage = FixedPcdGet32 (PcdAgesaFlashAreaBaseAddress) >> 24;
  ...
  return ((RomPage << 24) - RomBase);
}
/*
  Set the flash offset for Espi boot.
  @param[in]  FlashSize -  Size of physical flash part
