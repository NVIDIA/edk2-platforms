  ...
  )
{
  ...
  if (ChipSelectParameter->OrValue <= 1) {
    if (!mPspMailboxSpiMode) {
      ...
      mSpiCommunicationBuffer.SpiCommand[0].ChipSelect =
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (!EFI_ERROR (Status)) {
      if (!mPspMailboxSpiMode) {
        ...
        mSpiCommunicationBuffer.SpiCommand[0].Frequency =
  ...
  )
{
  ...
  if (!mPspMailboxSpiMode) {
    ...
    mSpiCommunicationBuffer.SpiCommand[0].OpCode    = Opcode;
    mSpiCommunicationBuffer.SpiCommand[0].BytesToTx = (UINT8)WriteBytes;
    if (WriteBytes != 0) {
      CopyMem (
        &mSpiCommunicationBuffer.SpiCommand[0].Buffer,
        WriteBuffer,
        WriteBytes
        );
    }

    mSpiCommunicationBuffer.SpiCommand[0].BytesToRx      = (UINT8)ReadBytes;
    mSpiCommunicationBuffer.CommandCount                 = 1;
    mSpiCommunicationBuffer.SpiCommunicationResult.Value = 0x0;
    mSpiCommunicationBuffer.ReadyToRun                   = TRUE;
    Status                                               = PspExecuteSpiCommand ();
    if (!EFI_ERROR (Status)) {
      if (mSpiCommunicationBuffer.SpiCommunicationResult.Value == 0x1000) {
        Status = EFI_INVALID_PARAMETER;
      } else {
        switch (mSpiCommunicationBuffer.SpiCommunicationResult.Field.Command0Result) {
          ...
          case SPI_COMMAND_COMPLETED:
            ...
            if (ReadBytes > 0) {
              CopyMem (
                ReadBuffer,
                &mSpiCommunicationBuffer.SpiCommand[0].Buffer[0] + WriteBytes,
                ReadBytes
                );
            }
