...
{
  if (ProcessorIndex < FixedPcdGet32 (PcdCoreCount)) {
    return TRUE;
  }
...
{
  return 2000000000; // 2 GHz, ToDo: update for Big-Little config
}
  ...
  )
{
  ...
  ProcessorCount = FixedPcdGet32 (PcdCoreCount);
...
{
  return FixedPcdGet32 (PcdCoreCount);
}
